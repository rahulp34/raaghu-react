...
describe("RdsCompOrganizationTree", () => {
  ...
  it("renders the component correctly", () => {
    ...
    expect(screen.getByText("NEW-ROOT-UNIT")).toBeInTheDocument();
    expect(screen.getAllByText("test1")).toHaveLength(2);
    expect(screen.getAllByText("child")).toHaveLength(2);
    const subUnitElement = screen.getAllByText("SUB-UNIT");
  ...
  it("renders icons correctly", () => {
    render(<RdsCompOrganizationTree {...mockProps} />);
    const addIconElement = screen.getAllByTestId("add-icon");
    const editIconElement = screen.getAllByTestId("edit-icon");
    const deleteIconElement = screen.getAllByTestId("delete-icon");
    addIconElement.forEach((item)=>{
        expect(item).toBeInTheDocument();
    });
    editIconElement.forEach((item)=>{
        expect(item).toBeInTheDocument();
    });
    deleteIconElement.forEach((item)=>{
        expect(item).toBeInTheDocument();
    })
  });

  it("calls onDeleteNode when the delete icon is clicked", () => {
    render(<RdsCompOrganizationTree {...mockProps} />);

    fireEvent.click(screen.getAllByTestId("delete-icon")[0]);
    expect(mockProps.onDeleteNode).toHaveBeenCalledWith(expect.any(Number));
  });

  it("calls onNodeEdit when the edit icon is clicked", () => {
    render(<RdsCompOrganizationTree {...mockProps} />);
    fireEvent.click(screen.getAllByTestId("edit-icon")[0]);
    expect(mockProps.onNodeEdit).toHaveBeenCalledWith(expect.any(Object));
  });

  it("calls onCreateNode when the add icon is clicked", () => {
    render(<RdsCompOrganizationTree {...mockProps} />);

    fireEvent.click(screen.getAllByTestId("add-icon")[0]);
    expect(mockProps.onCreateNode).toHaveBeenCalledWith(expect.any(Object));
  });

  it("calls onCreateSubUnit when the new sub-unit button is clicked", () => {
    render(<RdsCompOrganizationTree {...mockProps} />);
    fireEvent.click(screen.getAllByTestId("new-sub-unit")[0]);
    expect(mockProps.onCreateSubUnit).toHaveBeenCalledWith(expect.any(Object));
  });
});
