...
import { deleteComment, getAllComments, getCommentById } from "../../../../libs/state-management/comments/comments-slice";

const Comments = () => {

  const tableHeaders = [
    { displayName: "Username", key: "username", datatype: "text", dataLength: 30, required: true, sortable: false },
    { displayName: "Entity Type", key: "entityType", datatype: "text", dataLength: 30, required: true, sortable: false },
    { displayName: "Text", key: "text", datatype: "text", dataLength: 30, required: true, sortable: false },
  ...
  const [tableData, setTableData] = useState([
    { username: 'Test', entityType: 'test entity', text: 'test Text', creationTime: '21/03/2023' }
  ]);
  ...
  function onActionSelection(data: any, actionId: any): void {
    ...
    actionId === 'edit' ? dispatch(getCommentById(item)) : dispatch(deleteComment(item));
}
    ...
    <div className="row">
      <div className="card p-2 h-100 border-0 rounded-0 card-full-stretch mt-3">
        <form>
          <div className="d-flex justify-content-between">
            <div className="form-group me-2">
              <RdsDatepicker DatePickerLabel="Select Start Date" type="advanced" onDatePicker={onStartDateSelection} ></RdsDatepicker>
            </div>
            <div className="form-group me-2">
              <RdsDatepicker DatePickerLabel="Select End Date" type="advanced" onDatePicker={onEndDateSelection} ></RdsDatepicker>
            </div>
            <div className="form-group me-2">
              <RdsInput inputType="text" label={"Username"} value={filterUserName} placeholder={"Enter Username"}
                onChange={(event) => onFilterUsername(event)}
              ></RdsInput>
            </div>
            <div className="form-group me-2">
              <RdsInput inputType="text" label={"Entity Type"} value={filterEntityType} placeholder={"Enter Entity Type"}
                onChange={(event) => onFilterEntityType(event)}
              ></RdsInput>
            </div>
            <div className="form-group d-flex align-items-end">
              <div>
                <RdsButton colorVariant="primary" icon={'search'} type={"submit"} iconWidth={'16px'}
                  iconHeight={'20px'} onClick={(event) => searchByInputValues(event)}
                ></RdsButton>
              </div>
            </div>
          </div>
        ...
        <RdsCompDatatable tableHeaders={tableHeaders} tableData={tableData} pagination={false} actions={actions}
          ...
          recordsPerPage={5} noDataTitle={'No Comments Available'}></RdsCompDatatable>
      </div>
    </div>
