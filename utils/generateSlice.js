...
// const imports = serviceFileContent.match(/import type /g)
// const GetStates = serviceFileContent.match(/\bget\w+(?=\s*\()/g);
...
const asyncThunks = methods.map((method) => {
  ...
  }else{return `export const ${methodName} = createAsyncThunk(
    '${ServiceName.toLowerCase()}/${methodName}',
    async () => {
  ...
  extraReducers: (builder) => {
    ${methods
      .map((method) => {
        ...
        return `builder.addCase(${thunkName}request.pending, (state) => {
        state.loading = true;
      });
...
// fs.writeFileSync(sliceFilePath, sliceFileContent, 'utf-8');
